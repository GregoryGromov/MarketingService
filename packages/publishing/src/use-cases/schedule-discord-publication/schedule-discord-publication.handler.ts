...
export class ScheduleDiscordPublicationHandler
  implements ICommandHandler<ScheduleDiscordPublicationCommand, { id: string; status: string }>
{
  constructor(
  ...
  async execute(
    command: ScheduleDiscordPublicationCommand,
    ...
    if (channelId !== 'channel_discord') {
      throw new Error(
        `Adaptation ${adaptation.id} belongs to ${adaptation.channelId} and cannot be scheduled for Discord`,
      );
    }
...
function normalizeScheduledChannelId(channelId: string): 'channel_discord' | string {
  return channelId === 'discord' ? 'channel_discord' : channelId;
}
