...
import { ScheduleBlogPublicationCommand } from './schedule-blog-publication.command.js';

@CommandHandler(ScheduleBlogPublicationCommand)
export class ScheduleBlogPublicationHandler
  implements ICommandHandler<ScheduleBlogPublicationCommand, { id: string; status: string }>
{
  constructor(
  ...
  async execute(
    command: ScheduleBlogPublicationCommand,
    ...
    if (channelId !== 'channel_blog') {
      throw new Error(
        `Adaptation ${adaptation.id} belongs to ${adaptation.channelId} and cannot be scheduled for Blog`,
      );
    }
...
function normalizeScheduledChannelId(channelId: string): 'channel_blog' | string {
  return channelId === 'blog' ? 'channel_blog' : channelId;
}
