...
export interface SelectFirstKeywordRelatedQueriesResult {
  artifactId: string;
  keyword: string;
...
function readDerivedKeywordCandidates(
  payload: SeoBriefJsonValue,
  runId: string,
): {
  ...
  const keyword = asString(record.keyword)?.trim();
  const rawCandidates = Array.isArray(record.similarSearchQueries)
  ...
  const candidates = rawCandidates
    ...
    .map((item) => ({
      query: normalizeSearchQuery(asString(item.query) ?? ''),
      source: normalizeQuery(asString(item.source) ?? ''),
      sourceText: normalizeQuery(asString(item.sourceText) ?? asString(item.query) ?? ''),
      reason: normalizeQuery(asString(item.reason) ?? ''),
    }))
    .filter((item) => item.query.length > 0 && item.sourceText.length > 0);
...
@CommandHandler(SelectFirstKeywordRelatedQueriesCommand)
export class SelectFirstKeywordRelatedQueriesHandler
  implements
    ICommandHandler<SelectFirstKeywordRelatedQueriesCommand, SelectFirstKeywordRelatedQueriesResult>
{
  constructor(
  ...
  async execute(
    command: SelectFirstKeywordRelatedQueriesCommand,
    ...
    const derivedArtifact = [...artifacts]
      .reverse()
      .find((artifact) => artifact.artifactType === 'first_keyword_serp_derived_keywords');
    const { keyword, candidates } = readDerivedKeywordCandidates(
      derivedArtifact?.payload ?? null,
      run.id,
    );
    ...
    const artifact = SeoBriefArtifact.create({
      ...
      artifactType: 'first_keyword_related_query_selection',
      payload,
    });
