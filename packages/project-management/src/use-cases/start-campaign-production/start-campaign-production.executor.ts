...
export interface StartCampaignProductionExecutionResult {
...
interface PersistedSourceValidationResult {
  campaign: Campaign;
...
@Injectable()
export class StartCampaignProductionExecutor {
  private readonly logger = new Logger(StartCampaignProductionExecutor.name);
  ...
  async execute(
    campaignId: string,
    workflowRunId: string,
  ): Promise<StartCampaignProductionExecutionResult> {
    let snapshot!: SourceValidationSnapshot;

    this.logger.log(
      `Source check started: campaign=${campaignId} workflowRun=${workflowRunId}`,
    );
    ...
    try {
      ...
      const persistedResult = await this.transaction.run<PersistedSourceValidationResult>(
        async ({
          campaignRepository,
      ...
      if (persistedResult.approvalItem) {
        this.eventBus.publishAll(persistedResult.approvalItem.pullEvents());
      }
      this.eventBus.publishAll(persistedResult.campaign.pullEvents());

      this.logger.log(
        `Source check completed: campaign=${snapshot.campaignId} workflowRun=${workflowRunId} outcome=${validationResult.outcome} qualityCheck=${persistedResult.qualityCheckResult.id} approvalItem=${persistedResult.approvalItem?.id ?? 'none'}`,
      );

      return {
        workflowRunId,
        qualityCheckResultId: persistedResult.qualityCheckResult.id,
        outcome: validationResult.outcome,
        approvalItemId: persistedResult.approvalItem?.id ?? null,
      };
    } catch (error) {
      ...
      this.logger.error(
        ...
        `Source check failed: campaign=${campaignId} workflowRun=${workflowRunId}`,
      );
      throw error;
    }
  }
