...
let ApproveAdaptationHandler = class ApproveAdaptationHandler {
    channelAdaptationRepository;
    adaptationVersionRepository;
    eventBus;
    constructor(channelAdaptationRepository, adaptationVersionRepository, eventBus) {
    ...
    async execute(command) {
        ...
        await this.adaptationVersionRepository.deleteByAdaptationIdExcept(adaptation.id, adaptation.approvedVersionId ? [adaptation.approvedVersionId] : []);
        this.eventBus.publishAll(adaptation.pullEvents());
    }
};
ApproveAdaptationHandler = __decorate([
    CommandHandler(ApproveAdaptationCommand),
    __metadata("design:paramtypes", [ChannelAdaptationRepository,
        AdaptationVersionRepository,
        EventBus])
], ApproveAdaptationHandler);
export { ApproveAdaptationHandler };
//# sourceMappingURL=approve-adaptation.handler.js.map