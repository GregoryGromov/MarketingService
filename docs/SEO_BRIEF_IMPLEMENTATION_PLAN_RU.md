...
- [seo-briefing/README.md](/Users/grigorijgromov/Desktop/Overnight/Projects/MarketingService/marketing-service/docs/seo-briefing/README.md)
- [seo-briefing/13_FALLBACK_AND_RERUN_POLICY_RU.md](/Users/grigorijgromov/Desktop/Overnight/Projects/MarketingService/marketing-service/docs/seo-briefing/13_FALLBACK_AND_RERUN_POLICY_RU.md)
...
- `Phase 11` это operational hardening и удобство ежедневной работы, но ее лучше дробить на `11A`, `11B`, `11C`.
...
Лучше делать не одним этапом, а тремя подшагами:

1. `Phase 11A` — read-only UI и trace visibility.
2. `Phase 11B` — reruns и manual review actions.
3. `Phase 11C` — hardening, limits, deduplication и cost control.
...
5. Добавляем read-only просмотр scores, final brief и evidence pack.
...
1. Добавляем rerun:
   ...
   - stage rerun
   - regenerate brief
2. Добавляем manual review статусы:
   - `needs_manual_review`
   - `rejected`
   - `ready_for_regeneration`
3. Добавляем ручное изменение balance slider и повторный пересчет выбора.
4. Привязываем UI и backend-команды к fallback/rerun policy.

### Что делаем в 11C

1. Дорабатываем cache policy по DataForSEO.
2. Добавляем deduplication похожих run-ов.
3. Добавляем per-stage metrics и duration tracking.
4. Добавляем cost dashboard:
   - DataForSEO cost
   - LLM token cost
   - total run cost
5. Добавляем ограничения:
   - max initial keywords
   - max suggestion seeds
   - max clusters to score
   - max deep analysis clusters
   - max fallback attempts
6. Улучшаем retry/idempotency.
7. Добавляем regression tests на scoring formulas и selection logic.
...
- после `11A`: UI со step timeline, brief preview и evidence trail;
- после `11B`: rerun конкретного шага, rerun whole run, manual review и brief regeneration;
- после `11C`: cost dashboard, dedup protections, limits и regression safety.
...
После этого уже можно давать маркетологу реальный workflow. `Phase 11` делать после первых живых кейсов и накопления обратной связи, причем идти в порядке `11A -> 11B -> 11C`, а не пытаться закрыть все одним большим PR.
